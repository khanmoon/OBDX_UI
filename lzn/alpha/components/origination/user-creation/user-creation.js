  ...
  return function(rootParams) {
    ...
    self.loginFlowPost = function(refNoHeader) {
      ...
      UserCreationModel.createLoginConfig(JSON.stringify(loginConfigPayload), refNoHeader).done(function() {
        ...
        UserCreationModel.createLoginConfig(JSON.stringify(loginConfigPayload), refNoHeader).done(function() {
          ...
          var coappExisting = self.registrationCompulsory() && self.productDetails().requirements && self.productDetails().requirements.noOfCoApplicants > 0 && self.applicantDetails()[1].applicantType() === "customer";
          var isCreditCard = self.registrationCompulsory() && self.productDetails().productClassName === "CREDIT_CARD";
    ...
    self.updateEmail = ko.computed(function() {
      if (self.applicantDetails()[0].contactInfo) {
        self.userCreationPayload().username = self.applicantDetails()[0].contactInfo.emails[0].email();
