define([
  ...
  function fetchCurrentBrand() {
    return isBrandFetched ? Promise.resolve() : BaseService.getInstance().fetch({
      url: "brands/current",
      showMessage: false
    }).then(function(data) {
      if (data.assetDTO) {
  ...
  function setDashboard(userData) {
    ...
    if (userData.dashboardResponse) {
      for (i = 0; i < userData.dashboardResponse.dashboardDTOs.length; i++) {
        if (userData.dashboardResponse.dashboardDTOs[i].segment === Constants.userSegment) {
  ...
  function onSuccess(data, resolve) {
    ...
    setConstants(data.userProfile.homeEntity, segment, data.userProfile.timeZoneDTO.offset);
