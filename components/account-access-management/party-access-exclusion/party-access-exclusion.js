define([
    ...
    return function viewModel(rootParams) {
        ...
        self.finalPayload = { partyAccountAccessDTOs: "" };
        var getNewKoModel = function () {
            var KoModel = ExclusionModel.getNewModel();
            return ko.mapping.fromJS(KoModel);
        };
        self.rootModelInstance = getNewKoModel();
        self.partyID = ko.observable(rootParams.rootModel.partyID());
        self.indexSelected = ko.observable(rootParams.rootModel.indexSelected());
        ...
        self.createAccess = function () {
            ...
            ExclusionModel.createAccessClone(ko.toJSON(self.finalPayload), self.party.value(), self.params.isCorpAdmin).done(function (data, status, jqXhr) {
        ...
        self.updateAccess = function () {
            ...
            self.finalPayLoadArray.push(self.getTDPayload());
            self.finalPayload.partyAccountAccessDTOs = self.finalPayLoadArray();
            ExclusionModel.updateAccessClone(ko.toJSON(self.finalPayload), self.party.value(), self.params.isCorpAdmin).done(function (data, status, jqXhr) {
        ...
        self.LoadTransactionMappingComponent = function () {
            ...
            var params = { isCorpAdmin: self.params.isCorpAdmin };
            rootParams.dashboard.loadComponent("account-transactions-mapping", params, self);
        };
        self.readAccess = function () {
            ...
            self.updatedTDExclusionNumberList([]);
            self.updatedLOANExclusionNumberList([]);
            ExclusionModel.readAccess(self.party.value(), self.params.isCorpAdmin).done(function (data) {
                self.showModuleToMap(true);
                if (data.partyAccountAccessDTOs !== undefined) {
                    for (var i = 0; i < data.partyAccountAccessDTOs.length; i++) {
        ...
        self.deleteAccess = function () {
            ...
            self.finalPayLoadArray.push(self.getTDPayload());
            self.finalPayload.partyAccountAccessDTOs = self.finalPayLoadArray();
            ExclusionModel.deleteAccess(ko.toJSON(self.finalPayload), self.party.value()).done(function (data, status, jqXhr) {
                self.showDeleteButton(true);
                self.httpStatus(jqXhr.status);
                self.transactionStatus(data);
                self.showConfirmationForCreate(true);
